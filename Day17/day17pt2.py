...
def count_neighbours(state, x, y, z, w):
    ...
    for i in range(-1, 2):
        curr_x = x + i
        if curr_x < 0 or curr_x >= len(state[0][0][0]):
            continue
        for j in range(-1, 2):
            curr_y = y + j
            if curr_y < 0 or curr_y >= len(state[0][0]):
                continue
            for k in range(-1, 2):
                curr_z = z + k
                if curr_z < 0 or curr_z >= len(state[0]):
                    continue
                for l in range(-1, 2):
                    curr_w = w + l
                    if curr_w < 0 or curr_w >= len(state):
                        continue
                    if curr_x == x and curr_y == y and curr_z == z and curr_w == w:
                        continue

                    # print(curr_z,curr_y,curr_x)
                    num_neighbours += state[curr_w][curr_z][curr_y][curr_x]
...
def expand_state(state):
    ...
    state.insert(0, [[[False for x in range(new_dimensions[0])] for y in range(new_dimensions[1])] for z in range(new_dimensions[2])])
    state.append([[[False for x in range(new_dimensions[0])] for y in range(new_dimensions[1])] for z in range(new_dimensions[2])])
...
def next_state(state):
    ...
    dim = len(state[0][0][0]), len(state[0][0]), len(state[0]), len(state)
